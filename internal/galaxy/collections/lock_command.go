...
// plain run then writes the file, while --frozen only compares against it.
...
// lockfile, on a backend runLock already opened and locked. cfg.Frozen is
// checked first so the stricter lockFrozen verdict also covers --dry-run.
func lockWithState(ctx context.Context, cfg *config.Config, runtime *infra.Infra, state *installState, start time.Time) error {
	...
	// truthful zero: lock has no per-collection failure, and a save failure is
	// not counted there, matching install and warm.
...
// lockFrozen fails with helpers.ErrLockfileDrift when lf differs from the
...
// saveLockSnapshot is the one save rule lockDryRun and lockFrozen share, so
// lockFrozen under --dry-run saves exactly as lockDryRun does: through
