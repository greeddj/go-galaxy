...
`unknown key`, `[project] has neither collections nor roles`) exits `2` the
same way, and so does a collection version constraint semver cannot parse
...
file is missing too, or cannot be read, it exits `2`, while one that is not
YAML, or not TOML, still yields a key, since the key covers its bytes as they
are and a `galaxy.toml` is never decoded there. The fallback applies only to an absent
