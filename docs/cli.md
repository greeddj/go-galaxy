...
- `--requirements-file, -r`, also spelled `--role-file` (`$GO_GALAXY_REQUIREMENTS_FILE`,
  ...
  as `requirements.yml`, and nothing sniffs the content. Unset - neither the flag nor either
...
- `--lock-file` (`$GO_GALAXY_LOCK_FILE`) - defaults to `galaxy.lock`
  beside the requirements file, whichever one was picked
...
file is not parsed or validated, a `galaxy.toml` never decoded, so a malformed
requirements file still yields a key, and unlike the canonical lockfile hash,
any edit to it, whitespace and comments included, changes the key. A
requirements file that is missing or cannot be read exits `2`. A lockfile that
exists but fails to load is an error (`6`), never a fallback.
