...
- `install` (`i`) - install the collections and the roles of the requirements file - `galaxy.toml` or `requirements.yml`; see [galaxy.toml](configuration.md#galaxytoml) for which one a run reads - as `ansible-galaxy install -r` does; there is no separate role subcommand. Collections go under `--download-path`, roles under `--roles-path` (see [install options](#install-options)). Roles install after the collections and only when every collection level succeeded, so a collection failure never leaves roles half-installed against a broken tree; a run with no `roles:` entries never creates the roles directory. Each role is reported on its own line (`Installed: role <name> == <version>`, `Skipping install, already installed: role <name>@<version>`, `Failed: role <name> == <version> error: ...`), and the completion line counts roles only when the run had any, so a collections-only run reads as it always did.
- `lock` (`l`) - resolve and write `galaxy.lock` for reproducible CI. See [lock](#lock) below for its `--check` drift gate and its `--dry-run` preview.
...
`--cache-dir` is the one of them a `galaxy.toml` can also set, through
`cache_dir` in its `[tool.go-galaxy]` table.
...
flags plus the S3 flags. `outdated` registers the collection flag set plus the
S3 flags too, and `lock` the same with `--check` in place of `--frozen`, but
neither ever registers the signature-verification flags -
...
global options, `--requirements-file` and the S3 flags, listed separately under
...
- `--cache-dir` (`$GO_GALAXY_CACHE_DIR`, `$ANSIBLE_GALAXY_CACHE_DIR`) - the local cache directory.
  Unset, `cache_dir` in `[tool.go-galaxy]` of the run's `galaxy.toml` is read next (a relative
  path resolved against the file's directory), then `[galaxy] cache_dir` in `ansible.cfg`, then
  `$HOME/.cache/go-galaxy`; see [The local cache](caching.md#the-local-cache).
- `--server` (`$GO_GALAXY_SERVER`) - **Breaking change:** `$ANSIBLE_GALAXY_SERVER`
  ...
  `$GO_GALAXY_SERVER` (or pass `--server`) to keep the old behavior. A `galaxy.toml` supplies
  `server_list` through its `[[tool.go-galaxy.servers]]` entries, read in file order: they take
  the place of `[galaxy] server_list` and of the `[galaxy_server.<id>]` sections, which are then
  not read at all, while `$ANSIBLE_GALAXY_SERVER_LIST` still outranks them and
  `ANSIBLE_GALAXY_SERVER_<ID>_URL`, `_TOKEN` and `_VALIDATE_CERTS` still override an entry key by
  key; `--server=<id>` selects one of them. The file has no single-server key: the implicit
  server still comes from this flag, `[galaxy] server` or `$ANSIBLE_GALAXY_SERVER`.
- `--token` (`$GO_GALAXY_TOKEN`) - Galaxy API token for the single effective server;
  ...
  from a configuration file - `ansible.cfg` or `galaxy.toml` - rather than from
  you (`galaxy server token destination came from a configuration file`), and
  refused the same way when that server's certificate verification was disabled
  by the file rather than by you (`galaxy server certificate verification was
  disabled by a configuration file for a token it did not supply`) - see
  [--token](servers-and-auth.md#--token) for the remedy each half needs and why a
  section's own `token` key does not authorize an override either. A `token`
  written as `${VAR}` in `[[tool.go-galaxy.servers]]` is yours in the same sense,
  since whoever set the variable supplied it, and is held to the same rule; a
  literal token in either file is that file's own and exempt.
...
- `--roles-path` (`$GO_GALAXY_ROLES_PATH`, `$ANSIBLE_ROLES_PATH`) - the directory roles install into,
  ...
  Both warnings are printed only when the requirements file carries a non-empty roles list, the
...
- `--requirements-file, -r`, also spelled `--role-file` (`$GO_GALAXY_REQUIREMENTS_FILE`,
  ...
  collections and the roles lists alike. The file's format is decided by its extension alone: a
  `.toml` path, in any case, is read as [galaxy.toml](configuration.md#galaxytoml), any other path
  as `requirements.yml`, and nothing sniffs the content. A `galaxy.toml` also carries the run's
  settings in its `[tool.go-galaxy]` table - lockfile, cache directory, metrics file, worker
  counts, servers and S3 cache, each named under the option that reads it below - loaded before
  any other source, so a file that does not decode, breaks the schema or names an unset `${VAR}`
  is the first error a run reports (exit `2`). Unset - neither the flag nor either
  variable, where a variable exported empty counts as set and names the empty path - the run reads
  `./galaxy.toml` when it is a regular file and `./requirements.yml` otherwise, without examining
  that second candidate, so a directory holding neither file fails exactly as it always did. With
  both present it reads `galaxy.toml` and warns on stderr: `galaxy.toml and requirements.yml are
  both present in the current directory; using galaxy.toml and ignoring requirements.yml (name one
  with --requirements-file to choose)`. A `galaxy.toml` that is not a regular file (a directory, a
  fifo) is skipped with its own warning and `requirements.yml` is read instead. The picked file is
  where `galaxy.lock` is looked for beside and the path the project registry records; see
  [Discovery](configuration.md#discovery) for the whole rule and why a world-writable directory
...
- `--workers` (`$GO_GALAXY_WORKERS`) - number of concurrent workers. Unset, `workers` in
  `[tool.go-galaxy]` of the run's `galaxy.toml` is read next, and with neither it derives from the
  CPU this process is permitted to use (a container quota, not the node's core count), floored at 2
  and capped at 16. A value is accepted from `1` up to that permitted CPU count, itself floored at
  2 - so a 1- or 2-CPU runner still accepts `2`. Outside that range, in either direction, one
  warning is printed to stderr and the derived default is used instead, never a usage error; the
  warning names its source, `--workers (or $GO_GALAXY_WORKERS)` or `[tool.go-galaxy] workers in
  <path>`. A value that is not an integer at all is a different matter: on the flag it fails the
  flag parse itself and still exits `2`, and in the file a quoted `"4"`, a float or a boolean is a
  schema error (`[tool.go-galaxy] workers is not an integer`, exit `2`).
...
- `--download-workers` (`$GO_GALAXY_DOWNLOAD_WORKERS`) - number of concurrent artifact downloads
  ...
  a temp file, never an extraction). Unset, `download_workers` in `[tool.go-galaxy]` of the run's
  `galaxy.toml` applies when it is `1` or more, and with neither it derives from that same
  ...
  connection pool. A non-positive value, from the flag or the file, falls back to that same
  default rather than erroring; a file value that is not a TOML integer is a schema error (`2`).
...
- `--refresh` (`$GO_GALAXY_REFRESH`) - re-resolve against the configured Galaxy servers instead of reusing
  ...
  `lock --check` it does have an effect - see the `lock` command entry above. For a git source a branch or
...
- `--no-deps` (`$GO_GALAXY_NO_DEPS`) - installs only what the requirements file names. For collections
...
- `--offline` (`$GO_GALAXY_OFFLINE`) - fail on any network access (cached state only). A git source
  ...
  together with an S3 bucket - `--s3-bucket`, `$GO_GALAXY_S3_BUCKET` or `bucket` in
  `[tool.go-galaxy.s3]`, `--offline` from its own variable included - is refused as a usage error
  (`2`, `--offline cannot be combined with an S3 cache bucket: the S3 cache is reached over the
  network`) while the configuration is built, before any cache backend opens, on every command that
  takes both - `install`, `warm`, `lock` and `outdated`. An offline run uses the local cache.
- `--lock-file` (`$GO_GALAXY_LOCK_FILE`) - the lockfile to read or write. Unset, `lock_file` in
  `[tool.go-galaxy]` of the run's `galaxy.toml` (a relative path resolved against the file's
  directory), else `galaxy.lock` beside the requirements file.
- `--frozen` (`$GO_GALAXY_FROZEN`) - the lockfile is law, for `install` and `warm` (`outdated` accepts the flag and warns that it does nothing; `lock` does not take it, see `--check` below). `install`/`warm` resolve FROM the lockfile instead of the network: no version listing, no metadata fetch, just the pinned entries. A Galaxy entry the cache lacks is downloaded from its locked `download_url` with no metadata request at all, unless the run verifies signatures (`--keyring`), which reads the version metadata the server's signatures ride on and downloads from the URL that names. A `download_url` that fails fails that collection like any other download, with no fallback to the metadata; one that is not its server's own artifact URL (see `lock` above) fails the run as an invalid lockfile (`6`) before any request, and so does a lockfile an older release wrote, whose Galaxy entries carry no `download_url`, until `lock` rewrites it. A collection entry pinned by a SHA256 (a Galaxy or url collection) is installed only from an artifact whose SHA256 matches that pin, and an installed copy recorded with another SHA256 is installed again rather than kept. A cached artifact that does not match is evicted and downloaded again once from its origin (not under `--offline`, which keeps the only local copy; and a Galaxy entry fetched through its `download_url` is downloaded again without being evicted first, so only bytes that match the pin replace the cached copy); a downloaded artifact that does not match, or a cached one under `--offline`, fails that collection, and the run fails with the integrity code (`7`) once the work in flight has finished: `warm` finishes every other collection, and `install` finishes the current dependency level and starts no later one (see [Bounded recovery](architecture.md#bounded-recovery)). A git entry is pinned by its commit rather than by a SHA256 (see [The lockfile](architecture.md#the-lockfile)): a frozen `install`/`warm` installs it from the cache, and a cache that lacks its artifact fetches exactly that commit and rebuilds it. A git or Galaxy role entry is pinned the same way: with the artifact cached, a frozen run touches no network for it; on a cache miss the role is fetched by its pinned commit from the lockfile's repository, and a remote that serves a different commit for it fails the run with the integrity code (`7`). A url role entry is pinned by its origin bytes' SHA256, like a url collection, and its cached artifact, repacked from those bytes, is keyed by that pin: with it cached a frozen run touches no network for the role, and on a cache miss the URL is downloaded again and must hash to the pin, or the run fails with the integrity code (`7`). Under `--offline` a cache miss, collection or role, fails that entry, and the run exits with the install-failure code (`5`) like any other per-item failure. A `roles:` entry that drifted from its locked line - a different Galaxy version, a different repository or ref, a missing entry - is a lockfile mismatch (`6`).
- `--check` (`$GO_GALAXY_CHECK`), `lock` only - compare a fresh resolve with the lockfile on disk and fail on any difference (`6`) instead of writing it. `lock` cannot resolve from a file it is about to write, so it resolves fresh - exactly as a plain `lock` run does, including reusing a cached resolve - which makes `lock --check` a network path, unlike `install`/`warm` `--frozen`. See the `lock` command entry above for the comparison's exact contract.
- `--metrics-file` (`$GO_GALAXY_METRICS_FILE`) - emit JSON run report; unset, `metrics_file` in
  `[tool.go-galaxy]` of the run's `galaxy.toml`, a relative path resolved against the file's
  directory (see [Metrics](metrics.md)).
...
S3 cache options (a non-empty bucket selects the S3 backend). Each of them also reads its key in
the `[tool.go-galaxy.s3]` table of the run's `galaxy.toml` - `bucket`, `region`, `prefix`,
`access_key`, `secret_key`, `endpoint`, `session_token`, `path_style_disabled` - when the flag and
its variables are unset, a variable exported empty counting as set; a bucket from any source
without both keys from any source is refused (`2`, `s3 cache requires access and secret keys when
an S3 bucket is configured`). See [S3 Cache](caching.md#s3-cache-optional) for the file's shape:
...
- `--s3-path-style-disabled` (`$GO_GALAXY_S3_PATH_STYLE_DISABLED`) - switch to virtual-hosted-style
  ...
  which is what the flag disables. In the file it is a TOML boolean, `path_style_disabled = true`,
  read when the flag and its variable are unset.
...
- `--dry-run` (`$GO_GALAXY_DRY_RUN`) - report what `install`, `warm`, or `lock` would do, without
  ...
  exactly as it fails a real run, and a would-fail verdict or `lock --check` drift keeps its own
...
- `--requirements-file, -r`, also spelled `--role-file` (`$GO_GALAXY_REQUIREMENTS_FILE`,
  `$ANSIBLE_GALAXY_REQUIREMENTS_FILE`) - mounted for one reason: a `galaxy.toml` can name the cache
  to clean, through `cache_dir`, the `[tool.go-galaxy.s3]` table and the `[[tool.go-galaxy.servers]]`
  entries, and those are read from its `[tool.go-galaxy]` table exactly as `install` reads them.
  They are all `cleanup` takes from the table: it mounts neither `--lock-file` nor
  `--metrics-file`, nor either worker flag, so `lock_file`, `metrics_file`, `workers` and
  `download_workers` are neither taken nor credited, and the `--verbose` line
  `Galaxy.toml <file> supplied: ...` names only `cache_dir`, the `s3.*` keys and `servers`.
  The file is picked as `install` picks it - a set flag or variable verbatim, else `./galaxy.toml`
  when it is a regular file, else `./requirements.yml` - so discovery runs here too, with the same
  `galaxy.toml and requirements.yml are both present` warning. Nothing else is read from it: the
  `collections:` and `roles:` that decide what is reachable come from each recorded project's own
  requirements path, reloaded from the registry, never from the current directory's file. An
  explicitly named file that does not exist contributes nothing rather than failing, as does a
  `requirements.yml`, which has no settings table; a `galaxy.toml` that does not decode, breaks the
  schema or names an unset `${VAR}` anywhere in `[tool.go-galaxy]`, a key `cleanup` does not take
  included, fails the run (`2`) before anything is deleted.
- `--cache-dir` (`$GO_GALAXY_CACHE_DIR`, `$ANSIBLE_GALAXY_CACHE_DIR`) - unset, `cache_dir` in
  `[tool.go-galaxy]` of that file, then `[galaxy] cache_dir` in `ansible.cfg`, then the default.
...
**Upgrade note (galaxy.toml):** the project registry records the absolute path of the requirements file a project installed from, a `galaxy.toml` included, in the same field a `requirements.yml` path always went into. A release that predates `galaxy.toml` reads that field without looking at the extension, so its `cleanup` parses a recorded `galaxy.toml` as YAML - which reads `[project]` as a bare list holding one collection named `project` - refuses it and exits `2` for every project on that cache, deleting nothing - the fail-closed outcome, chosen over a stale-record reading that would have counted the project's roots as absent and removed what they protect. Upgrade every binary that shares a cache - an S3 bucket, a shared cache directory, an `action.yml` pinned to an older release - before any project on it adopts `galaxy.toml`; until then that cache's `cleanup` keeps failing and keeps everything.

**Upgrade note (lockfile schema 5):** every Galaxy collection in `galaxy.lock` now carries the `download_url` its artifact is fetched from, and a lockfile holding one is written as `schema_version: 5`. A lockfile an earlier release wrote has none, so every command that loads it - `install --frozen`, `warm --frozen`, `lock --check`, `hash`, `tree`, `explain` and `outdated` - refuses it as invalid (exit `6`) with a message naming the fix: run `go-galaxy lock` once and commit the result. The rewrite changes the file's `go-galaxy hash`, so a CI cache keyed on it starts from its restore keys once, and a release that predates schema 5 refuses the rewritten file: upgrade every binary that reads a project's lockfile together, an `action.yml` pinned to an older release included.

**Upgrade note (lock --check):** `lock --frozen` is now `lock --check`, read from `$GO_GALAXY_CHECK`; the gate itself is unchanged. `lock` no longer registers `--frozen`, so `lock --frozen` exits `2` as an undefined flag, and `$GO_GALAXY_FROZEN` no longer reaches `lock`: a CI step that relied on the variable to turn `lock` into a gate now writes the lockfile and passes, so spell `--check` or set `$GO_GALAXY_CHECK`. A `lock` run's metrics report no longer carries `frozen`. `install`, `warm` and `outdated` keep `--frozen` as they were.
...
- `--requirements-file, -r`, also spelled `--role-file` (`$GO_GALAXY_REQUIREMENTS_FILE`,
  `$ANSIBLE_GALAXY_REQUIREMENTS_FILE`) - the requirements file, `galaxy.toml` or `requirements.yml`
  by its `.toml` extension alone, picked exactly as `install` picks it: a set flag or variable
  (an empty value included) names the file verbatim, and unset, `./galaxy.toml` is read when it is
  a regular file, else `./requirements.yml`, with the same stderr warning when both are present
  or `./galaxy.toml` is not a regular file (see [install options](#install-options)). These three
  commands build no configuration, so each prints that warning itself, before its output.
- `--lock-file` (`$GO_GALAXY_LOCK_FILE`) - defaults to `lock_file` in the
  `[tool.go-galaxy]` table of a `galaxy.toml` requirements file (a relative
  path resolved against the file's directory), else `galaxy.lock` beside the
  requirements file, whichever one was picked. Reading that key loads the whole
  table by the one rule every command applies to it, so a `galaxy.toml` that
  does not decode, breaks the schema or names an unset `${VAR}` anywhere in
  `[tool.go-galaxy]` exits `2` on all three commands, `explain` included, before
  the lockfile is opened. The table is not loaded when the flag or its variable
  is set at all, an exported-empty value included (an empty value then falls to
  `galaxy.lock` beside the requirements file), so an unset `${VAR}` then fails
  none of the three; `tree` still reads its roots from the file, expanding
  nothing, and so still exits `2` on one that does not decode or breaks the
  schema. An absent `galaxy.toml` contributes nothing.
...
the SHA256 of the requirements file's bytes - the file the flag or discovery
picked, `galaxy.toml` or `requirements.yml` - exactly as they are on disk. The
key itself never parses the file, so a `requirements.yml` that is not YAML
still yields one, and unlike the canonical lockfile hash, any edit to it,
whitespace and comments included, changes the key. A `galaxy.toml` is decoded
once before that, to find `lock_file`, so one that is not TOML, breaks the
schema or names an unset variable exits `2` instead of yielding a key over its
bytes, unless `--lock-file` or its variable is set at all, an exported-empty
value included, in which case the file is never decoded; one that loads is
then hashed byte for byte like any other file. A requirements file that is
missing or cannot be read exits `2`. A lockfile that exists but fails to load
is an error (`6`), never a fallback.
